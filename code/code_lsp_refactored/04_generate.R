...
files <- list.files(path = ckPheDir, pattern = glob2rx("*.rda"), full.names = TRUE)
print(length(files))
...
if (!dir.exists(pheDir)) {
  dir.create(pheDir)
}


for (yToDo in 1:length(phenYrs)) {
  l01 <- matrix(NA, numPix, 1)
  l02 <- matrix(NA, numPix, 1)
  l03 <- matrix(NA, numPix, 1)
  l04 <- matrix(NA, numPix, 1)
  l05 <- matrix(NA, numPix, 1)
  l06 <- matrix(NA, numPix, 1)
  l07 <- matrix(NA, numPix, 1)
  l08 <- matrix(NA, numPix, 1)
  l09 <- matrix(NA, numPix, 1)
  l10 <- matrix(NA, numPix, 1)
  l11 <- matrix(NA, numPix, 1)
  l12 <- matrix(NA, numPix, 1)
  l13 <- matrix(NA, numPix, 1)
  l14 <- matrix(NA, numPix, 1)
  l15 <- matrix(NA, numPix, 1)
  l16 <- matrix(NA, numPix, 1)
  l17 <- matrix(NA, numPix, 1)
  l18 <- matrix(NA, numPix, 1)
  l19 <- matrix(NA, numPix, 1)
  l20 <- matrix(NA, numPix, 1)
  l21 <- matrix(NA, numPix, 1)
  l22 <- matrix(NA, numPix, 1)
  l23 <- matrix(NA, numPix, 1)
  l24 <- matrix(NA, numPix, 1)

  for (i in 1:numChunks) {
    cc <- sprintf("%03d", i)
    cfile <- paste0(ckPheDir, "/chunk_phe_", cc, ".rda")
    log <- try(load(cfile), silent = FALSE)
    if (inherits(log, "try-error")) next

    if (i == numChunks) {
      chunks <- c((chunk * (i - 1) + 1):numPix)
    } else {
      chunks <- c((chunk * (i - 1) + 1):(chunk * i))
    }

    chunkStart <- chunks[1]
    chunkEnd <- chunks[length(chunks)]

    l01[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 1)]
    l02[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 2)]
    l03[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 3)]
    l04[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 4)]
    l05[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 5)]
    l06[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 6)]
    l07[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 7)]
    l08[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 8)]
    l09[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 9)]
    l10[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 10)]
    l11[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 11)]
    l12[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 12)]
    l13[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 13)]
    l14[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 14)]
    l15[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 15)]
    l16[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 16)]
    l17[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 17)]
    l18[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 18)]
    l19[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 19)]
    l20[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 20)]
    l21[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 21)]
    l22[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 22)]
    l23[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 23)]
    l24[chunkStart:chunkEnd, ] <- pheno_mat[, (24 * (yToDo - 1) + 24)]
  }
  ...
  if (!dir.exists(pheDirYear)) {
    dir.create(pheDirYear)
  }
