...
GetSiteShp <- function(fileSR, cLong, cLat){
  ...
  geog_crs <- CRS(SRS_string ="EPSG:4326")
  # utm_crs = raster(fileSR[1])@crs
  utm_crs <- raster::crs(raster::raster(fileSR[1])) # otherwise uses terra's crs() namespace
  print(paste(geog_crs, ', ', utm_crs))
...
# NOTE
# defines NA raster template with correct spatial extent/resolution/crs
GetBaseImg <- function(fileSR, siteWin, outDir, save=TRUE){
  
  # Base Image
  for(i in 1:1000){ # try loading 1000 candidate scenes into rasters
    log <- try({img1 <- raster::raster(fileSR[i])},silent=T)
    if(inherits(log,'try-error')) next
    
    try(temp <- raster::intersect(img1,siteWin),silent=T)
    if(temp@extent[1]>0) break
  }
  img1 <- raster::crop(img1,siteWin) # crop to site window
  ...
  for(i in 1:numImg){
    log <- try({temp <- raster::raster(fileSR[sam[i]])},silent=T)
    if(inherits(log,'try-error')){
      imgBase[[i]] <- img1
    }else{
      imgBase[[i]] <- raster::raster(fileSR[sam[i]])
    }
  }
  
  for(i in 1:numImg){
    log <- try(raster::compareRaster(imgBase[[i]],img1,extent=F,rowcol=F),silent=T)
    if(inherits(log,'try-error')){
      imgBase[[i]] <- raster::projectRaster(imgBase[[i]],img1)    
    }
    log <- try(imgBase[[i]] <- raster::crop(imgBase[[i]],siteWin),silent=T)
    if(inherits(log,'try-error')){
      imgBase[[i]] <- img1
    }
  }
  imgBase$fun <- mean; 
  imgBase$na.rm <- T
  imgBase <- do.call(mosaic,imgBase) # average 50 scenes together to get consistent spatial extents
  raster::values(imgBase) <- NA # clears all pixels to NA
  
  if(save==TRUE){
    # Save Base Image
    raster::writeRaster(imgBase, filename=paste0(outDir,'/base_image.tif'), format="GTiff", overwrite=TRUE)  
  }
