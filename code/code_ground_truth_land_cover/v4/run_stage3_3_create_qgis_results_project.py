...
# preferred display order; anything found on disk outside this list still loads,
...
def discover_frameworks(cls_root, site, year):
    ...
    for entry in sorted(os.listdir(cls_root)):
        ...
        if not os.path.isdir(directory):
            continue
        if not glob.glob(
            os.path.join(directory, f"classification_{entry}_{site}_*_{year}.tif")
        ):
            continue
...
def style_classification(layer):
    ...
    classes = [
        QgsPalettedRasterRenderer.Class(
            code, QColor(CLASS_COLORS[code]), f"{code} {CLASS_LABELS[code]}"
        )
...
def style_probability(layer, band, colour):
    ...
    layer.setRenderer(
        QgsSingleBandPseudoColorRenderer(layer.dataProvider(), band, shader)
    )
...
def build_framework_group(project, root, framework, cls_dir, site, year, tiles):
    ...
    for tile, role in tiles.items():
        path = os.path.join(
            cls_dir, f"classification_{framework}_{site}_{tile}_{year}.tif"
        )
    ...
    for tile, role in tiles.items():
        path = os.path.join(
            cls_dir, f"class_probability_{framework}_{site}_{tile}_{year}.tif"
        )
        if not os.path.exists(path):
            continue
...
def build_rgb_group(project, root, cfg, data, tiles):
    ...
    Inputs:  project, root - QGIS objects; cfg - parsed config; data - site data
             directory; tiles - dict of {tile: role}
    Outputs: int, number of layers added
    """
    product = cfg["products"]["rgb"]
    group = root.addGroup("RGB 10cm")
    added = 0
    for tile, role in tiles.items():
        path = os.path.join(
            data, product["folder"], product["pattern"].format(tile=tile)
        )
...
def main():
    ...
    for framework, cls_dir in discover_frameworks(cls_root, site, year):
        added = build_framework_group(
            project, root, framework, cls_dir, site, year, tiles
        )
        found.append((framework, added))

    rgb_added = build_rgb_group(project, root, cfg, data, tiles)
    ...
    if not found:
        print(
            "no classification rasters found - run run_stage3_1_random_forest_ground_truth_classification.py first"
        )
    print(f"run {run}")
    ...
    print(
        "Probability layers load unchecked - four bands times five tiles would obscure the classification. Tick the one being interrogated."
    )
    print(
        "Classification rasters also carry an embedded colour table, so they render correctly in any GDAL reader without this project."
    )
    print(
        "Frameworks are discovered from disk - run another one and re-run this script; nothing here needs editing."
    )
