...
def summarize_field(rows, limit_m):
    ...
    return {
        ...
        "ci95_m": [round(median_offset - 1.96 * se, 3), round(median_offset + 1.96 * se, 3)],
...
def check_inventory(audit, cfg, site_dir):
    ...
    audit.add(
        ...
        {"bounds": bounds, "overlaps": overlaps, "duplicates": len(bounds) != len(set(map(tuple, bounds.values())))},
    )
    return bounds
...
def check_georeferencing(audit, cfg, site_dir):
    ...
    audit.add(
        ...
        {"crs_found": {k: len(v) for k, v in crs_seen.items()}, "expected": expected_crs, "utm_zone": utm_zone},
        blocker=True,
    )
    ...
    significant = [
        f"{tile}/{label}"
        for tile, entry in coreg.items()
        for label, s in entry.items()
        if label != "chm_savi" and s.get("exceeds_limit")
    ]
    audit.add(
        9,
    ...
    for tile in tiles:
        for index in cfg["products"]["vi"]["indices"]:
            ...
            with rasterio.open(path) as ds:
                ...
                if index in ("SAVI", "NDVI") and not (
                    -1.0 <= float(arr.min()) and float(arr.max()) <= 1.0
                ):
                    out_of_range.append(key)
    evi_out = [
        k
        for k, v in vi_stats.items()
        if k.endswith("/EVI") and not (-1.0 <= v["min"] and v["max"] <= 1.0)
    ]
    audit.add(
        13,
...
def check_phenocam(audit, cfg, site_dir):
    ...
    for n in (1, 2):
        ...
        inside = [
            t for t, b in bounds.items()
            if b.left <= easting <= b.right and b.bottom <= northing <= b.top
        ]
        tiles_hit += inside
        cams[name] = {
            "lat": float(lat),
...
def check_deferred(audit):
    for num, name, reason in (
        ...
        (20, "Per-class pixel counts under the reference rules", "needs Step 1 outputs"),
        ...
        ("22a", "Distribution of the 1 m and block confidence layers", "needs Step 1/3"),
...
def main():
    ...
    if audit.blocking_failures:
        sys.exit(
            "STEP 0 FAILED - blocking checks: "
            + ", ".join(f"#{c['check']}" for c in audit.blocking_failures)
        )
