...
def pooled_thresholds(cfg, site_dir, tiles, bins):
    ...
    s1c = cfg["stage1_6_detect_shadows"]
    ...
    for tile in tiles:
        rgb, _, _ = rgb_at_scale(
            tile_paths(cfg, site_dir, tile)["rgb"],
            cfg["stage1_5_generate_features"]["texture_scale_m"],
        )
    ...
    return (
        _percentile_from_hist(luma_hist, luma_edges, s1c["luma_percentile"]),
        _percentile_from_hist(blue_hist, blue_edges, s1c["blue_percentile"]),
    )
...
def main():
    ...
    out_dir = resolve(cfg["results_root"], "stage1_data_and_features", "shadow")
    out_dir.mkdir(parents=True, exist_ok=True)

    all_tiles = list(cfg["tiles"])
    pool_tiles = (
        [t for t, role in cfg["tiles"].items() if role == "train"]
        if s1c["pool_over"] == "train"
        else all_tiles
    )

    print(f"pass 1: pooling luma over {len(pool_tiles)} tile(s) [{s1c['pool_over']}]")
    threshold, blue_threshold = pooled_thresholds(
        cfg, site_dir, pool_tiles, s1c["hist_bins"]
    )
    mode = s1c["blue_shift_mode"]
    print(f"pooled p{s1c['luma_percentile']} luma threshold = {threshold:.3f}")
    if mode == "percentile":
        print(
            f"pooled p{s1c['blue_percentile']} blue-fraction threshold = {blue_threshold:.4f}"
        )
    else:
        print("blue-shift rule: absolute B > R (spec wording)")
    print()

    scale = cfg["stage1_5_generate_features"]["texture_scale_m"]
    ...
    for tile in all_tiles:
        ...
        with rasterio.open(paths["chm"]) as ds:
            grid = dict(
                transform=ds.transform, crs=ds.crs, width=ds.width, height=ds.height
            )
        ...
        chm_valid = np.where(
            chm_valid > cfg["thresholds"]["chm_max_valid_m"], 0.0, chm_valid
        )
        ...
        out_profile = dict(
            profile, dtype="uint8", count=1, nodata=255, compress="deflate"
        )
        with rasterio.open(
            out_dir / f"shadow_mask_ref_{site}_{tile}_{year}.tif", "w", **out_profile
        ) as ds:
            ds.write(mask, 1)
        ...
        summary.append(row)
        print(
            f"[{tile}] shadow 0.6m={row['shadow_pct_06m']:>6.2f}%  1m={row['shadow_pct_1m']:>6.2f}%"
            f"  ->tree={row['resolved_to_tree_pct']:>5.2f}%  ->nodata={row['masked_to_nodata_pct']:>5.2f}%"
        )
    ...
    spec = {
        ...
        "blue_shift_rule": (
            f"chromatic b = B/(R+G+B) > p{s1c['blue_percentile']} (pooled) = {blue_threshold:.4f}"
            if mode == "percentile"
            else "B > R (absolute)"
        ),
    ...
    (out_dir / f"shadow_{site}_{year}_spec.json").write_text(
