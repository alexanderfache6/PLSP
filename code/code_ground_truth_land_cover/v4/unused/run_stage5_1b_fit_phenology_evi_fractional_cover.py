"""Stage 5_1b - RF-B with EVI2 magnitude features added to the timing set.

THIS IS A CONTROLLED VARIANT OF STAGE 5_1, NOT A REPLACEMENT. Stage 5_1 fits 10
timing-only features and, at SRER 2022, beat a constant-mean baseline by 3.9%
cross-validated and by -0.3% on held-out tiles - that is, not at all
(stage5_results.md section 9). The open question is whether timing carries no
signal or whether the PLSP product carries signal that timing alone omits. This
script answers it by adding the three EVI2 magnitude layers and changing NOTHING
ELSE: same QA mask, same stage 4 targets, same folds, same models, same pixels.
Any difference in score is attributable to the three added features alone.

Maps 13 LSP metrics - 10 timing, 3 EVI2 magnitude - to four class fractions, one
prediction per 3 m Planet pixel. This is a REGRESSOR, not a classifier: RF-A
classifies 1 m pixels and stage 4 counts them into fractions; this step predicts
those fractions directly from phenology (section 4.3).

THE ADDED FEATURES ARE MAGNITUDE, NOT TIMING, which is the whole point. EVImax
is peak greenness, EVIamp is the seasonal swing, and EVIarea integrates
greenness over the cycle. A bare block and a grass block can green up on the
same date and differ enormously in how green they get, so magnitude is exactly
the axis timing cannot see. At SRER these three layers are 98.4% valid at 2022,
far broader than the 70.22% QA mask, so they cost no pixels: the site-wide map
covers the identical 7,803,438 pixels as the timing-only run and the two are
directly comparable.
...
THE OUTPUT LABEL IS NOT THE INPUT RUN. `--run 5` names the stage 4 aggregation
directory that supplies the targets and is unchanged; the outputs go to
`run5_timing_evi` so the timing-only run 5 is never overwritten and the two sit
side by side for comparison. Conflating the two would have sent this script
looking for a `stage4_aggregation/run5_timing_evi/` that does not exist.

Outputs -> `stage5_phenology_model_prediction/run{N}_timing_evi/`:

    stage5_1_report_{SITE}_{YEAR}.json - every score, per fold, pooled, and on
        the held-out test tiles, for both models. THE 5_1 NAME IS KEPT ON
        PURPOSE so that run_stage5_2 reads this run with no edit at all; the
        directory carries the distinction and the report names its own
        feature_set. Distinguishing them by filename too would buy nothing and
        cost a fork of 5_2.
...
ARGUMENTS, identical to stage 5_1 so the two are driven the same way:
    ...
    --run
        ...
        targets. UNLIKE 5_1 IT NAMES THE INPUT DIRECTORY ONLY:
        targets are read from `stage4_aggregation/run{N}/` and
        results are written to
        `stage5_phenology_model_prediction/run{N}_timing_evi/`.
    --framework
    ...
    --target
        ...
        Which stage 4 fraction estimate to regress on.
    --phenology-year
...
fit has completed. That is exactly how the first attempt at this run failed.

    conda activate LCSC

    # the normal run
    python run_stage5_1b_fit_phenology_evi_fractional_cover.py config/srer_2022.json --run 5 --framework C

    # scores only, no site-wide map
    python run_stage5_1b_fit_phenology_evi_fractional_cover.py config/srer_2022.json --run 5 --framework C --no-predict

Then, with no change to either script, note that the run LABEL is what the
downstream scripts take, not the stage 4 run number:
...
# THE THREE LAYERS THAT MAKE THIS 5_1b RATHER THAN 5_1. Layer 9 EVImax is the
# maximum EVI2 in the cycle, layer 10 EVIamp its amplitude, layer 11 EVIarea the
# integral. THEIR SCALES ARE NOT 1 AND NOT EQUAL TO EACH OTHER: EVImax and
# EVIamp are 0.0001, EVIarea is 0.01, all read from PLSP_Layers.csv by number.
# Both product tiers spell all three with the specification name, so unlike
# layers 3 and 7 they need no alternate-spelling fallback - verified against
# every SRER year 2017 to 2025.
MAGNITUDE_LAYER_NUMBERS = [9, 10, 11]
FEATURE_SET_NAME = "timing_evi"
...
def read_timing_and_magnitude_feature_stack(dataset, specification):
    """The 13 features - 10 timing, 3 EVI2 magnitude - masked and scaled.
    ...
    trap 1). The magnitude layers share that fill value and are masked by the
    ...
    the scale per layer by number is what makes adding the magnitude group a
    one-constant change rather than new scaling code.

    FEATURE ORDER IS TIMING, THEN DURATIONS, THEN MAGNITUDE, and the magnitude
    ...
    """
    scaled_layers = {}
    for layer_number in RAW_TIMING_LAYER_NUMBERS + MAGNITUDE_LAYER_NUMBERS:
    ...
    for layer_number in MAGNITUDE_LAYER_NUMBERS:
...
def main():
    parser = argparse.ArgumentParser(description="Fit RF-B on timing plus EVI2 magnitude features, PlanetScope phenology to fractional cover.")
    ...
    # THE INPUT RUN AND THE OUTPUT LABEL ARE DELIBERATELY DIFFERENT. Targets
    # come from stage 4 run 5, which knows nothing about feature sets; results
    # go to run5_timing_evi so the timing-only run 5 survives untouched beside
    # them. Deriving the label from args.run keeps the pair in step.
    output_label = f"run{args.run}_{FEATURE_SET_NAME}"
    ...
    print(f"RF-B - timing plus EVI2 magnitude to fractional cover - {site}")
    print("=" * SEVENTY)
    print(f"ground truth year {ground_truth_year} (stage 4 run {args.run}, framework {args.framework}, target {args.target})")
    print(f"feature set {FEATURE_SET_NAME}, writing to {output_label}")
    ...
    with xr.open_dataset(netcdf_path, mask_and_scale=False) as netcdf_dataset:
        ...
        feature_stack, feature_names = read_timing_and_magnitude_feature_stack(netcdf_dataset, layer_specification)
